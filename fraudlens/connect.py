"""Native cognis-connect emit for fraudlens — forward findings to any platform.

Maps fraudlens's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    fraudlens ... --format json | fraudlens-emit --to stix
    fraudlens-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('rule', '') + ' - ' + rec.get('description', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'fraud')
